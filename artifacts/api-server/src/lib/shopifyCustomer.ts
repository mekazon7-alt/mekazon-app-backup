...
/**
 * Delete a Shopify customer record and its personal data (for Apple's required
 * in-app account deletion). Past ORDERS are retained by Shopify for legal /
 * accounting reasons, but the customer's PII is removed. Returns true on success.
 * Requires the Admin token to have the `write_customers` scope.
 */
export async function deleteCustomer(customerId: string): Promise<boolean> {
  if (!SHOPIFY_ADMIN_CONFIGURED) return false;
  const data = await adminGraphql<{
    customerDelete: {
  ...
  return Boolean(data.customerDelete.deletedCustomerId);
}